...
$address_err = $phn_no_err = $email_err =    "";
$phn_no_invalid = $email_invalid = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($email) && !empty($address) && !empty($phn_no)) {

        // database connection
        require_once "include/connection.php";

        $add_details  = "INSERT INTO `contact_us` ( `address` , `phn` , `email`) VALUES( '$address' , '$phn_no', '$email' )";
        $result = mysqli_query($conn, $add_details);

        if ($result) {
            $address = $phn_no = $email =  "";
            echo "<script>
                              ...
                              <div class="card-body pt-5 shadow">
                                   ...
                                   <form method="POST" action=" <?php htmlspecialchars($_SERVER['PHP_SELF']) ?>">
                                        ...
                                        <div class="form-group">
                                             <label>Address :</label>
                                             <input type="text" class="form-control" value="<?php if (empty($address)) {
                                                                                        echo $address_err;
                                                                                    } else {
                                                                                        echo $address;
                                                                                    }  ?>" name="address">
                                        ...
                                        <div class="form-group">
                                             ...
                                             <input type="text" class="form-control" value="<?php if (empty($phn_no)) {
                                                                                        echo $phn_no_invalid;
                                                                                    } else {
                                                                                        echo $phn_no;
                                                                                    }  ?>" name="phn">
